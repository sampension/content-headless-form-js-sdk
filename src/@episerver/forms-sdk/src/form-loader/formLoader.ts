...
export class FormLoader<T extends FormContainer> {
  ...
  getForm(key: string, language: string): Promise<T> {
    return new Promise<T>((resolve, reject) => {
      this.client
        .get(`${ApiConstant.apiEndpoint}/${key}`, { locale: language })
        .then((response: T) => {
          resolve(response);
        })
        .catch((error: any) => {
    ...
    return new Promise<T>((resolve, reject) => {
      let query: string = `
            query FormQuery($key: String, $language: [Locales]) {
                FormContainer(locale: $language, where: { Key: { eq: $key } }) {
                    items {
                        Key
                        Locale
                        Properties
  ...
  private convertFirstLetterToLowerCase(data: any): any {
    ...
    if (data && isObject && !Array.isArray(data)) {
      return Object.keys(data).reduce((accumulator, key) => {
        ...
        accumulator[normalizedKey] = isObject
          ? this.convertFirstLetterToLowerCase(data[key])
          : data[key];
        return accumulator;
      }, {} as any);
